...
namespace RecipeApp
{
    public partial class MainRecipePage : ContentPage
    {
        ...
        public async Task<String> getData()
        {
            ...
            var response = await client.GetStringAsync("145.24.222.221/search.php?input=%category=all");
        ...
        private async void add_bookmark(Recipe recipe) {
            ...
            if (bookmark_id_list.Contains(recipe.ID) == false) 
            {
				...
				var response = await client.GetStringAsync("145.24.222.221/bookmark.php?user=" + Global.username + "&add=" + recipe.ID);
            ...
            else 
            {
                ...
                var response = await client.GetStringAsync("145.24.222.221/bookmark.php?user=" + Global.username + "&remove=" + recipe.ID);
        ...
        private async Task<List<BookmarkItem>> Bookmark_check(HttpClient client) {
            var response = await client.GetStringAsync("145.24.222.221/bookmark.php?user=" + Global.username);
